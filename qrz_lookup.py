...
from PyQt5.QtGui import QCursor, QDesktopServices, QFont, QPixmap
...
def _make_map_html(lat: float, lon: float, internet_available: bool = True) -> str:
    """Generate folium map HTML with a single marker pin."""
...
class _ImageLoader(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            if not px.isNull():
                self.image_loaded.emit(px.scaledToHeight(126, Qt.SmoothTransformation))
        except Exception:
            pass
...
class _QRZInfoSection(QWidget):
    ...
    def __init__(self, embed_moddate: bool = True, parent=None):
        super().__init__(parent)
        self._img_loader: Optional[_ImageLoader] = None
        self._build(embed_moddate)

    def _build(self, embed_moddate: bool) -> None:
...
class QRZLookupDialog(QDialog):
    ...
    def __init__(self, panel_background: str = "#f5f5f5",
                 panel_foreground: str = "#333333", parent=None):
        super().__init__(parent)
    ...
    def _setup_ui(self) -> None:
        ...
        self.qrz_info = _QRZInfoSection(embed_moddate=False, parent=self)
        info_block = QVBoxLayout()
        info_block.setSpacing(0)
        info_block.setContentsMargins(0, 0, 0, 0)
        info_block.addWidget(self.qrz_info)
        moddate_row = QHBoxLayout()
        moddate_row.addStretch(2)
        moddate_row.addWidget(self.qrz_info.lbl_moddate, 1, Qt.AlignTop | Qt.AlignRight)
        info_block.addLayout(moddate_row)
        main.addLayout(info_block)
        main.addStretch()
...
class StatRepDetailDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        self.qrz_info = _QRZInfoSection(embed_moddate=True, parent=self)
        self.qrz_info.image_width_ready.connect(self._adjust_for_image_width)
        main.addWidget(self.qrz_info)

        # Section 2: StatRep metadata
        s2_widget = QWidget()
        s2_layout = QVBoxLayout(s2_widget)
        s2_layout.setContentsMargins(4, 4, 4, 4)
        s2_layout.setSpacing(2)
        s2_hdr = QLabel("Status Report Details")
        s2_hdr.setFont(QFont("Arial", 14, QFont.Bold))
        self.lbl_date_posted = QLabel(); self.lbl_date_posted.setFont(QFont("Arial", 11))
        self.lbl_global_id   = QLabel(); self.lbl_global_id.setFont(QFont("Arial", 11))
        self.lbl_delivery    = QLabel("")
        self.lbl_delivery.setFont(QFont("Arial", 11))
        # TODO: Query CommStat server for delivery count. Do not query if record is over 48 hours old.
        for w in (s2_hdr, self.lbl_date_posted, self.lbl_global_id, self.lbl_delivery):
            s2_layout.addWidget(w)
        s2_layout.addStretch()
        main.addWidget(s2_widget)
        ...
        for col_idx, (label_text, _) in enumerate(STATUS_FIELDS):
            ...
            sq = QLabel()
            sq.setFixedHeight(27)
            sq.setStyleSheet("background-color:rgb(255,255,255); border:1px solid #444444;")
        ...
        # Action buttons
        btn_row = QHBoxLayout()
        brevity_note = QLabel("<b>Brevity Note:</b> Double-click brevity code, then click the Brevity button")
        brevity_note.setFont(QFont("Arial", 10))
        btn_row.addWidget(brevity_note)
        btn_row.addStretch()
        self.btn_delete = QPushButton("Delete")
        self.btn_delete.setStyleSheet(_btn_style("#dc3545"))
        self.btn_delete.setVisible(False)  # shown only if callsign matches local user
    ...
    def _load_statrep(self) -> None:
        ...
        # Show Delete only if this statrep belongs to the local user
        if self.callsign.upper() == _get_local_callsign().upper():
            self.btn_delete.setVisible(True)

        # Section 2 metadata (index 0 = datetime, index 1 = global_id)
        self.lbl_date_posted.setText(f"<b>Date Posted:</b>  {row[0]}" if row[0] else "<b>Date Posted:</b>")
        global_id = row[1] or 0
        self._global_id = global_id
        self.lbl_global_id.setText(f"<b>Global ID:</b>  {global_id}" if global_id else "<b>Global ID:</b>")

        if not global_id:
            self.lbl_delivery.setText("")
        ...
        for i, (label_text, _) in enumerate(STATUS_FIELDS):
            ...
            color_str, tip = STATUS_COLORS.get(val, ("rgb(255,255,255)", "No status"))
            sq.setStyleSheet(
                f"background-color:{color_str}; border:1px solid #444444;"
            )
            sq.setToolTip(tip)
    ...
    def _on_read_count(self, count: int) -> None:
        self.lbl_delivery.setText(f"<b>Delivered to:</b>  {count} CommStat users")
    ...
    def _on_delete(self) -> None:
        if not self._global_id or not self._backbone_url:
            return
        try:
            local_cs = _get_local_callsign()
        ...
        try:
            with sqlite3.connect(DB_PATH, timeout=10) as conn:
                conn.execute("DELETE FROM statrep WHERE sr_id = ?", (self.sr_id,))
                conn.commit()
...
class MessageDetailDialog(QDialog):
    ...
    def __init__(self, callsign: str, message_text: str,
                 ...
                 panel_background: str = "#f5f5f5",
                 panel_foreground: str = "#333333",
                 data_background: str = "#ffffff", parent=None):
        super().__init__(parent)
        self.callsign = callsign
    ...
    def _setup_ui(self) -> None:
        ...
        # Map (480x270) + message text, side by side
        lower = QHBoxLayout()
        lower.setSpacing(10)
        self.map_view = QWebEngineView()
        self.map_view.setFixedSize(480, 270)
        ...
        self.msg_text.setFixedSize(480, 270)
        ...
        for label, color in [
            ...
            b = QPushButton(label)
            b.setStyleSheet(_btn_style(color))
            # TODO: Button handlers to be implemented later
            btn_row.addWidget(b)
        main.addLayout(btn_row)
